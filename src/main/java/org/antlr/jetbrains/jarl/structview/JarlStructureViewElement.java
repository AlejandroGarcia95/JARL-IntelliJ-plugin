...
public class JarlStructureViewElement implements StructureViewTreeElement, SortableTreeElement {
	protected final PsiElement element;

	public JarlStructureViewElement(PsiElement element) {
	...
	public ItemPresentation getPresentation() {
		return new JarlItemPresentation(element);
	}

	@NotNull
	@Override
	public TreeElement[] getChildren() {
		if ( element instanceof JarlPSIFileRoot) {
			Collection<? extends PsiElement> funcs = XPath.findAll(JarlLanguage.INSTANCE, element, "/script/function/ID");
			List<TreeElement> treeElements = new ArrayList<>(funcs.size());
			for (PsiElement el : funcs) {
				treeElements.add(new JarlStructureViewElement(el));
			}
